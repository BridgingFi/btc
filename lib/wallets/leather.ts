...
type LeatherNetwork = (typeof LeatherNetworks)[number];
...
export class Leather implements Wallet {
  private _network: Network =
    (localStorage.getItem("leather_network") as Network) ?? "testnet";
  ...
  switchNetwork(network: Network): Promise<void> {
    ...
    return Promise.reject(
      new Error(
        "You need to change network in your Leather wallet and then reconnect",
      ),
    );
  }
  ...
  get balance(): Promise<Balance> {
    return this.accounts
      .then((accounts: any) => {
        if (accounts[0])
          return fetch(this.mempoolApiUrl(`/api/address/${accounts[0]}`));
      ...
      return Promise.all([this.accounts, this.publicKey])
        .then(([accounts, pubKeyStr]) => {
          ...
          if (accounts[0]) {
            ...
            return fetch(
              this.mempoolApiUrl(`/api/address/${accounts[0]}/utxo`),
            ).then(getJson);
          }
          throw new Error("wallet not connected");
        })
              ...
              utxos.map(async (utxo: any) => {
                return {
                  ...
                  nonWitnessUtxo: (p2ret.type as string).startsWith("w")
                    ? undefined
                    : await fetch(
                        this.mempoolApiUrl(`/api/tx/${utxo.txid}/hex`),
                      ),
        ...
        .then((selected) => {
          ...
          return this.instance
            ...
            .then((response: any) => {
              ...
              if (txid) return txid;
              const finalTx = btc.Transaction.fromPSBT(
                hex.decode(response.result.hex),
  ...
  signPsbt(psbtHex: string, options?: SignPsbtOptions): Promise<string> {
    const signAtIndex: number[] | undefined = options?.toSignInputs.map(
      (i) => i.index,
    );
