...
export class OKX extends UniSat {
  private _network: Network =
    (localStorage.getItem("okx_network") as Network) ?? "livenet";
  ...
  switchNetwork(network: Network): Promise<void> {
    ...
    this.requestAccounts().then((accounts: any) =>
      this.instance.emit("accountsChanged", accounts),
    );
  ...
  get publicKey() {
    return this._instanceTestnet
      ? this._instanceTestnet
          .getSelectedAccount()
          .then((result: any) => result?.publicKey)
  ...
  get balance(): Promise<Balance> {
    ...
    return this.accounts
      .then((accounts: any) =>
        fetch(
          `https://mempool.space/${this._network}/api/address/${accounts[0]}`,
        ),
      )
      .then(getJson)
      .then((result: any) => {
  ...
  signPsbts(psbtHexs: string[], options?: SignPsbtOptions): Promise<string[]> {
    return (this._instanceTestnet ?? this.instance).signPsbts(
      psbtHexs,
      options,
    );
  }
