...
export class WalletStandard implements Wallet {
  ...
  private _network: Network =
    (localStorage.getItem("satsconnect_network") as Network) ?? "livenet";
  constructor(wallet: WalletType | WSWallet) {
    ...
    this._wallet = wallet;
    this._provider = (wallet.features as SatsConnectFeature)[
      SatsConnectNamespace
    ].provider;
  }
  ...
  get balance(): Promise<Balance> {
    return this.accounts
      .then((accounts: any) =>
        fetch(mempoolApiUrl(`/api/address/${accounts[0]}`, this._network)),
      )
      .then(getJson)
      .then((result: any) => {
        return {
          ...
          inputsToSign: [
            {
              ...
              signingIndexes:
                ...
                Array.from(
                  {
                    length: btc.Transaction.fromPSBT(hex.decode(psbtHex), {
